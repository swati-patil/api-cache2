...
public class Memcached {
	...
	public static String fetchValueFromCache (String key) throws Exception {
		String result = "";
		cacheHandle = getInstance();
		result = (String) cacheHandle.get(Constants.CACHE_PREFIX+key);
		return result;
	}
	
	public static void setValueInCache (String key, String value) throws Exception {
		cacheHandle = getInstance();
		if (Constants.urlsToBeCached.contains(key))
			cacheHandle.set(Constants.CACHE_PREFIX+key, 3600, value);
		else {
			//ignore
